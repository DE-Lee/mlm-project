...
export const MapView = ({ width = 600, height = 600 }: MapViewProps) => {
  ...
  const { robots, selectedRobot } = useRobotStore();
  const { publishGoal } = useGoalPublisher(selectedRobot || 'robot1');
  const { publishInitialPose } = useInitialPose(selectedRobot || 'robot1');
  ...
  useEffect(() => {
    ...
    ROBOTS.forEach((robotConfig) => {
      ...
      // 선택된 로봇 강조
      if (selectedRobot === robotConfig.namespace) {
        ctx.strokeStyle = '#fff';
        ctx.lineWidth = 2;
        ctx.stroke();
      }
      ...
      ctx.fillText(robotConfig.name, pos.x, pos.y + robotRadius + 15);
  ...
  }, [mapImage, robots, selectedRobot, worldToPixel, mapMode, hoverPos, dragState, width, height]);
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if ((mapMode !== 'initialPose' && mapMode !== 'goal') || !selectedRobot) return;
  ...
  const handleMouseUp = () => {
    ...
    // 상태 초기화
    setDragState({ startPos: null, currentPos: null, isDragging: false, mode: 'none' });
    setMapMode('none');
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-2 flex-wrap">
        {/* 초기 위치 설정 버튼 */}
        <button
          onClick={() => setMapMode(mapMode === 'initialPose' ? 'none' : 'initialPose')}
        ...
        >
          {mapMode === 'initialPose' ? '클릭+드래그로 위치 설정...' : '초기 위치 설정'}
        </button>

        {/* 목표점 설정 버튼 */}
        <button
          onClick={() => setMapMode(mapMode === 'goal' ? 'none' : 'goal')}
          className={`px-4 py-2 rounded-lg font-medium transition-colors ${
            mapMode === 'goal'
        ...
        >
          {mapMode === 'goal' ? '클릭+드래그로 목표점 설정...' : '목표점 설정'}
        </button>
      ...
      {mapMode === 'initialPose' && (
        <div className="text-sm text-amber-400">
          맵에서 클릭 후 드래그하여 로봇의 초기 위치와 방향을 설정하세요
        </div>
      )}
      {mapMode === 'goal' && (
        <div className="text-sm text-green-400">
          맵에서 클릭 후 드래그하여 목표 위치와 방향을 설정하세요
        </div>
      )}
